...
try {
  ...
  throw new Error(
    "xml2js module not found. Please install it with 'npm install xml2js'"
  );
}
...
ocrsdk.prototype.processImage = function (filePath, settings, userCallback) {
  ...
  var req = this._createTaskRequest(
    "POST",
    "/processImage" + urlOptions,
    userCallback
  );
 ...
 */
ocrsdk.prototype.downloadResult = function (
  resultUrl,
  outputFilePath,
  userCallback
) {
 ...
 */
ocrsdk.prototype._createTaskRequest = function (
  method,
  urlPath,
  taskDataCallback
) {
